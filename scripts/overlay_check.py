...
# --- НОВЫЙ КОНТУР ПРЯМО ПО СКАНУ: внешняя огибающая r на каждой высоте MZ ---
MR_scan=np.zeros(len(MZ))
for i,zmm in enumerate(MZ):
    win=20 if 30<zmm<Hs-30 else 14
    sel=np.abs(t-zmm)<win
    if sel.sum()<40:
        MR_scan[i]=MR_scan[i-1] if i else 180
    else:
        MR_scan[i]=np.percentile(r[sel],99.3)
MR_scan=np.round(MR_scan).astype(int)
print("MR_scan (мм, обведено по скану off-08):")
print("const MR =["+",".join(f"{v:4d}" for v in MR_scan)+"];")
print("\nвысота(см)  скан R(см)  стар.мой(см)  разн(см)")
for i,zmm in enumerate(MZ):
    print(f"  {zmm/10:4.1f}     {MR_scan[i]/10:6.1f}      {MR[i]/10:6.1f}     {(MR_scan[i]-MR[i])/10:+5.1f}")
MR=MR_scan.astype(float)   # рисуем НОВЫЙ обведённый контур
